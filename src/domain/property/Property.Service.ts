...
import { Filters } from "@src/domain/property/Filters";
  ...
  static filter(properties: readonly Property[], filters: Filters) {
    return Effect.sync(() => {
      ...
      return properties.filter((house) => {
        if (type && !house.type.toLowerCase().includes(type.toLowerCase())) return false;
        if (location && !house.location.toLowerCase().includes(location.toLowerCase()))
          return false;
